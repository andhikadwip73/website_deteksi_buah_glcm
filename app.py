...
CLASS_MAPPING = {
    "Ripe_frames": ("Matang", "Layak / Matang"),
    "Ripe2_frames": ("Matang", "Layak / Matang"),
    "Overripe_frames": ("Terlalu Matang", "Tidak Ideal"),
    "Rotten_frames": ("Busuk", "Tidak Layak"),
}
...
def get_sample_image_path():
    ...
    if DATASET_DIR.exists():
        for image_path in DATASET_DIR.rglob("*"):
            if image_path.is_file() and allowed_file(image_path.name):
                return image_path
...
def classify_with_hsv_model(features, hsv_model):
    ...
    distances = {
        class_name: sqrt(
            sum(
                (value - center) ** 2
                for value, center in zip(vector, centroid)
            )
        )
        for class_name, centroid
        in hsv_model["centroids"].items()
    }

    best_class = min(distances, key=distances.get)

    scores = {
        name: exp(-distance * 8.0)
...
def aggregate_display_probabilities(probabilities, labels):
    ...
    for class_name in labels:
        label, status = CLASS_MAPPING.get(
            class_name,
            (class_name, "Perlu Dicek"),
        )
        ...
        grouped[key]["score"] += float(
            probabilities.get(class_name, 0)
        )
...
def predict_image(image_path):
    ...
    if hsv_model is None:
        return {
            "ready": False,
            "message": (
                "Model HSV belum tersedia. "
                "Jalankan python train_model.py terlebih dahulu."
            ),
            "hsv": hsv,
        }

    raw_class, probabilities = classify_with_hsv_model(
        hsv,
        hsv_model,
    )

    labels = hsv_model.get(
        "class_names",
        get_class_names(),
    )

    display_probabilities = aggregate_display_probabilities(
        probabilities,
        labels,
    )
